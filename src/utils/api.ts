...
const getGoodsByUUID = (cNo: string) => request.get<any>(`/Goods/getGoodsByUUID?cNo=${cNo}`)
...
const updateDealStatus1 = (orderId: string) => request.get<any>(`/deal/updateDealStatus1?orderId=${orderId}`)
const dealFilter = (size: number, page: number) => request.get<any>(`/deal/dealFilter?size=${size}&page=${page}`, )
